      ...
      <div className="container-shell flex h-[78px] items-center justify-between">
        ...
        <nav className="hidden items-center gap-8 lg:flex">
          {links.map(([href, cn, en]) => (
            <Link key={href} href={href} aria-current={pathname === href ? "page" : undefined} className={`text-sm font-bold transition ${pathname === href ? "text-pulse" : "text-ink/65 hover:text-ink"}`}>
              {cn}<span className="ml-1.5 text-[10px] uppercase tracking-wider opacity-50">{en}</span>
            </Link>
          ))}
        </nav>
        <Link href="/contact" className="hidden rounded-full bg-ink px-6 py-3 text-sm font-bold text-white transition hover:bg-pulse lg:block">
          开始合作
        </Link>
